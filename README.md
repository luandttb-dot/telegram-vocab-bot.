...
- Đổi model Gemini (nếu model mặc định bị Google ngừng hỗ trợ, sẽ báo lỗi
  `404 Not Found` trong log Actions): thêm secret `GEMINI_MODEL`.
  Cách lấy tên model đang hoạt động thật: dán link sau vào trình duyệt
  (thay `YOUR_KEY` bằng Gemini API key của bạn):
  `https://generativelanguage.googleapis.com/v1beta/models?key=YOUR_KEY`
  Tìm các model có `"generateContent"` trong `supportedGenerationMethods`,
  copy phần tên sau `models/` (ví dụ `gemini-3.7-flash`). Google đổi tên/rút
  model khá thường xuyên, nên đây là cách đáng tin cậy hơn đoán tên.
  Mặc định code hiện đang dùng `gemini-3.7-flash`.

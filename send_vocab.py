"""
Gửi 1 tin nhắn DUY NHẤT mỗi lần chạy: 1 đoạn văn tiếng Anh (do Gemini AI
viết) chứa các từ vựng của lần chạy đó, từ vựng được in đậm.
...
- Vì sheet có thể thay đổi bất cứ lúc nào, cam kết "không sót từ" áp dụng
  ...
  từ đầu ngày — đây là đánh đổi cần thiết để luôn phản ánh đúng sheet mới
  nhất mà không cần lưu trạng thái ở đâu khác.

MỖI LẦN CHẠY GỬI ĐÚNG 1 TIN NHẮN, CHỈ CÓ ĐOẠN VĂN (không header/emoji, không
ghi chú gì thêm):
- Gọi Gemini API (miễn phí, xem README.md) viết đoạn văn lồng ghép toàn bộ
  từ của lần chạy này, bôi đậm *từ* khi xuất hiện lần đầu, rồi gửi nguyên
  văn đoạn đó vào Telegram.
- Nếu gọi AI thất bại hoàn toàn (hết quota, lỗi mạng...), tin nhắn duy nhất
  đó sẽ chỉ liệt kê tên 50 từ (không có nghĩa/ví dụ) thay vì im lặng bỏ qua
  ngày hôm đó.
- Tin nhắn dài hơn giới hạn 4096 ký tự của Telegram sẽ tự tách làm nhiều
  tin liên tiếp — đây là yêu cầu kỹ thuật bắt buộc của Telegram, không phải
  script cố ý gửi nhiều tin.
...
def generate_paragraph(sample, max_retries=3):
    """
    ...
    Google (503 quá tải, 429 quá nhiều request, hoặc lỗi mạng), có chờ tăng
    dần giữa các lần thử (1s, 2s, 4s...).
    Trả về đoạn văn (str).
    """
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                wait_s = 2 ** (attempt - 1)
                print(
                    f"Lỗi khi gọi Gemini (lần thử {attempt}/{max_retries}): {e}. "
                    f"Chờ {wait_s}s rồi thử lại..."
                )
                time.sleep(wait_s)
                continue
...
def build_fallback_message(sample):
    """Khi AI lỗi hoàn toàn: chỉ liệt kê tên từ, không thêm header/emoji."""
    return ", ".join(w["word"] for w in sample)
...
def main():
    ...
    used_ai = False
    try:
        paragraph = generate_paragraph(final_list)
        used_ai = True
        message = paragraph
        parse_mode = "Markdown"
        ...
        message = build_fallback_message(final_list)
        parse_mode = None
